...
const validationSchema = z.object({
    input: z
        ...
        .max(12, 'خیلی طولانی است!'),
    select: z.string().min(1, 'لطفاً یک گزینه انتخاب کنید!'),
    multipleSelect: z
    ...
    date: z
        .date()
        .nullable()
        .refine((val) => val !== null, { message: 'تاریخ الزامی است!' }),
    time: z
        .date()
        .nullable()
        .refine((val) => val !== null, { message: 'زمان الزامی است!' }),
    singleCheckbox: z.boolean().refine((val) => val === true, {
        message: 'باید این گزینه را تیک بزنید!',
    }),
    multipleCheckbox: z
        .array(z.union([z.string(), z.number()]))
        .nonempty('حداقل یک گزینه را انتخاب کنید!'),
    radio: z.string().min(1, 'لطفاً یک گزینه انتخاب کنید!'),
    switcher: z.boolean().refine((val) => val === true, {
        message: 'باید این گزینه را فعال کنید!',
    }),
    upload: z.array(z.instanceof(File)).nonempty('حداقل یک فایل باید آپلود شود!'),
    segment: z.array(z.string()).nonempty('حداقل یک گزینه را انتخاب کنید!'),
})
