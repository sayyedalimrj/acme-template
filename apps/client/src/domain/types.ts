...
// ===========================================================================================
// Platform Admin (internal control layer) — Phase: platform-admin dashboard (mock-only).
//
// These are the FRONTEND-SAFE types for OUR internal team to manage the SaaS customers
// (tenants) behind the merchant stores. They intentionally model the Tenant → Site → Resource
// hierarchy from architecture.md and combine plugin/sync/security/subscription signals.
//
// SECURITY/PRIVACY (binding): no real PII, no real emails/phones (fake demo labels only), no
// secrets, no billing provider IDs, no plugin signing secrets, no WooCommerce credentials.
// MRR and limits are display-only mock numbers (no FX, no billing). See security-model.md.
// ===========================================================================================

/** Lifecycle of a SaaS customer/tenant from our side. */
export type PlatformTenantStatus = 'active' | 'trialing' | 'past_due' | 'canceled' | 'suspended';

/** Subscription state (display-only; no real billing). */
export type PlatformSubscriptionState = 'trialing' | 'active' | 'past_due' | 'canceled';

/** Plan tier (matches the merchant Plans module). */
export type PlatformPlanTier = 'starter' | 'growth' | 'pro' | 'managed';

/** Priority used for support + internal admin tasks. */
export type PlatformAdminPriority = 'low' | 'medium' | 'high' | 'urgent';

/** Internal task/workflow status (prepares the Workflow Operations board). */
...
/** Connection status of a managed site (mirrors the merchant SiteConnection status). */
export type PlatformSiteConnectionStatus = 'connected' | 'disconnected' | 'pending' | 'error';

/** Sync delivery state for a site's signed/event pipeline (mock). */
export type PlatformSyncState = 'healthy' | 'delayed' | 'stalled' | 'never';

/** Signed-sync verification state (maps to the signed delivery foundation). */
export type PlatformSignedSyncState = 'verified' | 'unsigned' | 'invalid' | 'disabled';

/** Security/audit signal kinds surfaced from the plugin/backend trust boundary. */
...
/** Severity of a security/audit signal. */
export type PlatformSecuritySeverity = 'info' | 'warning' | 'critical';

/** Display-only owner/contact label for a tenant (never real PII). */
export interface PlatformCustomerOwner {
  /** Fake, display-only owner label, e.g. "owner-label-1". */
  label: string;
  /** Clearly-fake demo email (example.test domain). */
  email: string;
  /** Display-only role label. */
  role?: string;
}

/** Plugin health for a single managed site. */
export interface PlatformPluginHealthSummary {
  pluginVersion: string;
  latestVersion: string;
  wooCommerceActive: boolean;
  /** 0–100 plugin health score. */
  score: number;
  /** Short, frontend-safe status note. */
  note?: string;
}

/** Signed-sync + event-bridge health for a single managed site. */
export interface PlatformSyncHealthSummary {
  state: PlatformSyncState;
  signed: PlatformSignedSyncState;
  lastSyncAt?: ISODate;
  /** Pending events in the local bridge queue (mock). */
  queuedEvents: number;
  /** Rejected/failed deliveries in the window (mock). */
  failures: number;
}

/** A WordPress/WooCommerce store managed under a tenant. */
export interface PlatformSiteSummary {
  ...
  /** Derived 0–100 site health (connection + plugin + sync). */
  healthScore: number;
}

/** Subscription summary for a tenant (display-only; no billing). */
export interface PlatformSubscriptionSummary {
  tier: PlatformPlanTier;
  state: PlatformSubscriptionState;
  /** Display-only monthly recurring revenue (mock; no FX/conversion). */
  mrr: Money;
  currency: CurrencyCode;
  /** Display-only renewal date. */
  renewsAt?: ISODate;
  /** Trial end date (when trialing). */
  trialEndsAt?: ISODate;
}

/** Aggregated support state for a tenant. */
export interface PlatformSupportSummary {
  openItems: number;
  priority: PlatformAdminPriority;
  /** Fake support owner label. */
  owner?: string;
  /** Display-only oldest-open age in hours (mock). */
  oldestOpenHours?: number;
}

/** Usage/limits snapshot for a tenant (mock; display-only). */
export interface PlatformUsageSummary {
  ...
  sitesUsed: number;
  sitesLimit: number;
  /** True when any metric is near/over its plan limit. */
  nearLimit: boolean;
}

/** A security/audit signal related to a tenant (and optionally a site). */
export interface PlatformSecuritySignal {
  ...
  /** Frontend-safe message (never contains secrets/values). */
  message: string;
  createdAt: ISODate;
  /** Whether our team already reviewed it (mock). */
  acknowledged: boolean;
}

/** A SaaS customer/tenant we manage. */
export interface PlatformTenant {
  id: string;
  /** Business / store-group name. */
  name: string;
  ...
  /** Derived 0–100 tenant health (site/plugin/sync + subscription + support + security). */
  healthScore: number;
  /** Display-only last activity timestamp. */
  lastActiveAt: ISODate;
  /** Short frontend-safe internal note placeholder. */
  internalNote?: string;
}

/** An internal operational task for our team (prepares the Workflow Operations board). */
export interface PlatformAdminTask {
  ...
  /** Fake assignee label. */
  owner?: string;
  /** Display-only due date. */
  dueAt?: ISODate;
  /** Related tenant/site (when applicable). */
  tenantId?: string;
  siteId?: string;
  /** The next operational action for our team. */
  nextAction: string;
  /** Why it is blocked (when status === 'blocked'). */
  blockedReason?: string;
}

/** Everything the Platform Admin overview screen needs. */
export interface PlatformAdminOverview {
  ...
  /** Highest-risk tenants (lowest health first), small slice. */
  atRiskTenants: PlatformTenant[];
  /** Sites with the worst sync/plugin health, small slice. */
  sitesAtRisk: PlatformSiteSummary[];
  /** Top next admin tasks. */
  topTasks: PlatformAdminTask[];
  /** Recent security signals (newest first), small slice. */
  recentSecuritySignals: PlatformSecuritySignal[];
  /** Subscription-state breakdown for a mini chart. */
