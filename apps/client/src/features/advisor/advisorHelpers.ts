...
const DISCLAIMER =
  ' (این پیشنهاد به بازبینی شما نیاز دارد و هیچ اقدامی به‌صورت خودکار انجام نمی‌شود.)';
...
const REPLY_BODY: Record<AdvisorReplyKind, string> = {
  sales:
    'بر اساس داده‌های فروشگاه شما، محصولات پرفروش را در صفحه نخست برجسته کنید و یک پیشنهاد فروش مکمل (cross-sell) برای آن‌ها بسازید. همچنین یک بازبینی قیمت روی محصولات کم‌فروش می‌تواند کمک‌کننده باشد.',
  inventory:
  ...
  generic:
    'می‌توانم درباره فروش، موجودی، کمپین‌ها، بهبود توضیح محصول و ایده‌های رسانه‌ای کمک کنم. بر اساس داده‌های فروشگاه شما، از بخش پیشنهادها می‌توانید گام بعدی را انتخاب کنید.',
};
