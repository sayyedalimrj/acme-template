...
describe('Settings language toggle', () => {
  it('renders Persian and English language options and switches locale', async () => {
    renderWithProviders(<SettingsScreen />);
    const fa = await screen.findByTestId('lang-fa', {}, { timeout: 4000 });
    const en = screen.getByTestId('lang-en');
    expect(fa).toBeTruthy();
    expect(en).toBeTruthy();
    // Switching to English updates visible UI strings (settings title becomes English).
    fireEvent.press(en);
